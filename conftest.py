...
# ... [Previous browser and authenticated_browser fixtures remain the same] ...
...
# def add_failure_overlay(driver, test_name, failed_step=None):
...
#         // Add border around the failing step
#         if (arguments[1]) {
#             var stepBorder = document.createElement('div');
#             stepBorder.style.position = 'fixed';
#             stepBorder.style.top = '0';
#             stepBorder.style.left = '0';
#             stepBorder.style.right = '0';
#             stepBorder.style.bottom = '0';
#             stepBorder.style.border = '4px solid red';
#             stepBorder.style.pointerEvents = 'none';
#             stepBorder.style.zIndex = '9999';
#             document.body.appendChild(stepBorder);
#         }
#     """, test_name, failed_step)
...
#                 # 2. Find recently interacted elements
#                 try:
#                     interacted_elements = driver.execute_script("""
#                         return Array.from(document.querySelectorAll('*')).filter(el => 
#                             el.querySelector(':focus') || 
#                             el.matches(':focus') || 
#                             el.matches(':active') ||
#                             el.matches('input:not([value=""])') ||
#                             el.matches('select:not([value=""])') ||
#                             el.matches('textarea:not(:empty)')
#                         );
#                     """)
#                     elements_to_highlight.extend(interacted_elements)
#                 except WebDriverException:
#                     pass

#                 # 3. Highlight all identified elements
...
#                 # 4. Add failure overlay with step information
#                 add_failure_overlay(driver, item.name, failed_step)
...
#                 driver.execute_script("""
#                     var overlay = document.getElementById('test-failure-overlay');
#                     if (overlay) overlay.remove();
#                     var borders = document.querySelectorAll('div[style*="border: 4px solid red"]');
